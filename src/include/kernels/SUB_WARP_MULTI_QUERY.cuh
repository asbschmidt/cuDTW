#ifndef sub_warp_DTW_MULTI_QUERY
#define SUB_WARP_DTW_MULTI_QUERY
...
void sub_warp_DTW_multi_query(
    ...
    for (index_t query_number = 0; query_number < num_queries; query_number++) {
        ...
        if (thid % group_size == 0)  penalty_diag = INFINITY;


        index_t counter = 1;
        value_t query_value = INFINITY;
        value_t new_query_value = cQuery[query_number*num_features+thid%group_size];
    ...
    penalty_here31 = (query_value-subject_value31) * (query_value-subject_value31) + min(penalty_here30, min(penalty_here31, penalty_temp0));
       ...
       for (index_t k = 3; k < lane+group_size-1; k++) {
           const index_t i = k-l%group_size;
            ...
            if (counter%group_size == 0) new_query_value = cQuery[query_number*num_features+i+2*(thid%group_size)-1];
        ...
        if(thid % group_size == group_size-1) Dist[query_number*num_entries+(32/group_size)*blockIdx.x+thid/group_size] = penalty_here31;
    }
