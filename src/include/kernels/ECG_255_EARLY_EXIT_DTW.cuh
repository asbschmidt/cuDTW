...
#ifndef ECG_255_EARLY_EXIT_DTW
#define ECG_255_EARLY_EXIT_DTW

// 8 values per thread no shared memory
template <
    typename index_t,
    typename value_t> __global__
void ECG_255_early_exit_DTW( // was DTW_fast_256_shuffle_kernel_no_shared_memory
    value_t * Subject,
    value_t * Dist,
    index_t num_entries,
    ...
    //} else {
        ...
        value_t subject_value0 = l == 0 ? 0 : Subject[base+8*l-1];
        value_t subject_value1 = Subject[base+8*l-0];
        value_t subject_value2 = Subject[base+8*l+1];
        value_t subject_value3 = Subject[base+8*l+2];
        value_t subject_value4 = Subject[base+8*l+3];
        value_t subject_value5 = Subject[base+8*l+4];
        value_t subject_value6 = Subject[base+8*l+5];
        value_t subject_value7 = Subject[base+8*l+6];
        ...
        value_t S_last = subject_value7; // Subject[base+num_features-1];
        S_last = __shfl_sync(0xffffffff, S_last, 31);
        value_t S_second_last = subject_value6; // // Subject[base+num_features-2];
        S_second_last = __shfl_sync(0xffffffff, S_second_last, 31);
        value_t S_third_last = subject_value5; // Subject[base+num_features-3];
        ...
        for (index_t k = 3; k < lane+WARP_SIZE-1; k++) {
            ...
            if (counter%32 == 0) {
                penalty_temp0 = min(min(min(min(min(min(min(min(penalty_left, penalty_here0), penalty_here1), penalty_here2), penalty_here3), penalty_here4), penalty_here5), penalty_here6), penalty_here7);
    ...
    if(thid == blockDim.x-1 && mask) {
        Dist[blid] = penalty_here7;
           if (penalty_here7 < *threshold)
                update_bsf_system(threshold, penalty_here7);
    }
